...
income_data <- 
  ...
  filter(!grepl(c("06000"), county_fips),
         !grepl(c("06003"), county_fips),
         grepl(c(" "), name))
```
...
covid_county_region_month_av_plot <- 
  ...
  #theme(axis.title.x = element_text("COVID Deaths per Positive Case (%) in CA County"))
...
tree_county_region_plot <- 
  ...
  geom_col(show.legend = FALSE) +
  scale_fill_viridis_d() #+
  #theme(axis.title.x = element_text("Tree Canopy Cover (%) in CA County"))

covid_county_region_month_av_plot / tree_county_region_plot
```
